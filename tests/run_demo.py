...
attacker = KeyboardAgent(reverse_vocab) if control_attacker else \
...
while not done:
    ...
    defender_action = defender.compute_action_from_dict(obs[AGENT_DEFENDER],
        infos[AGENT_DEFENDER]["action_mask"]) if not attacker_only else \
            null_action
    attacker_action = attacker.compute_action_from_dict(obs[AGENT_ATTACKER],
    ...
    logger.debug('Attacker has compromised the following attack steps so '
        'far:')
