...
from malpzsim.agents.searchers import BreadthFirstAttacker, DepthFirstAttacker
...
def test_pz(env: MalPettingZooSimulator):
...
def test_env_step(env: MalPettingZooSimulator) -> None:
    obs, info = env.reset()
...
def test_check_space_env(env : MalPettingZooSimulator) -> None: 
    attacker_space = env.observation_space("attakcer")
...
def test_attacker(env: MalPettingZooSimulator, attacker_class)-> None :
    obs, info = env.reset()
...
def test_env_multiple_steps(env: MalPettingZooSimulator) -> None:
    obs, info = env.reset()
    for _ in range(100):
        attacker_action = env.action_space("attacker").sample()
        defender_action = env.action_space("defender").sample()        
...
#test determinisme 
def test_determinisme(env: MalPettingZooSimulator):
   logger.debug("Run Parrallel seed test.")
   parallel_seed_test(env, num_cycles=5)
