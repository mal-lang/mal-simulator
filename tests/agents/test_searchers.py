...
def test_breadth_first_traversal_complicated(dummy_lang_graph: LanguageGraph) -> None:
    ...
    for level in (0, 3), (3, 7):
        assert set(expected_order[level[0] : level[1]]) == set(
            actual_order[level[0] : level[1]]
        ), 'Traversal order does not match expected breadth-first order'


def test_depth_first_traversal_complicated(dummy_lang_graph: LanguageGraph) -> None:
    r"""
                    node0 ______________
                  /       \             \
            node1          node2        node7
    ...
    assert (
        3 in actual_order[index1 + 1 : index1 + 3]
        and 4 in actual_order[index1 + 1 : index1 + 3]
    )

    # All children of 2 must come directly after it
    index2 = actual_order.index(2)
    assert (
        5 in actual_order[index2 + 1 : index2 + 3]
        and 6 in actual_order[index2 + 1 : index2 + 3]
    )
