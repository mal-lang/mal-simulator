...
class AssetSnapshot:
    """Structural fingerprint of a single asset, keyed by id."""

    type: str
    name: str
    associations: dict[str, frozenset[int]]


def snapshot_model(model: Model) -> dict[int, AssetSnapshot]:
    """Capture a structural fingerprint of every asset in the model, by id."""
    return {
        asset.id: AssetSnapshot(
            type=asset.type,
            name=asset.name,
            associations={
...
def reconcile_model_to_snapshot(
    model: Model, snapshot: dict[int, AssetSnapshot]
) -> tuple[
    set[ModelAsset],
    set[ModelAsset],
    ...
    for asset_id in target_ids - current_ids:
        asset_snapshot = snapshot[asset_id]
        new_asset = _add_asset_with_id(
            model,
            asset_type=asset_snapshot.type,
    ...
    for asset_id, asset_snapshot in snapshot.items():
        asset = model.assets[asset_id]
        current_associations = {
            field_name: frozenset(other.id for other in others)
            for field_name, others in asset.associated_assets.items()
        }
        for field_name in set(current_associations) | set(asset_snapshot.associations):
            current_ids_for_field = current_associations.get(field_name, frozenset())
            target_ids_for_field = asset_snapshot.associations.get(
...
def reset_model_effects(
    attack_graph: AttackGraph, model_snapshot: dict[int, AssetSnapshot]
) -> None:
