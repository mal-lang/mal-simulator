...
def attack_graph_to_nx_graph(
    attack_graph: AttackGraph, ttc_values: dict[AttackGraphNode, float]
) -> nx.DiGraph:
    """
...
def _find_path_to(
    ...
    paths = (
        _nx_shortest_path(attack_graph, nx_graph, start, goal_node, ttc_values)
...
def get_shortest_path_to(
    ...
    try:
        path, _ = _find_path_to(
            attack_graph, nx_graph, total_path, compromised_nodes, goal_node, ttc_values
        )
    ...
    planned_path = list(filter(lambda step: step not in compromised_nodes, total_path))
...
def get_shortest_paths_for_attacker(
    ...
    ttc_values = {
        n: node_ttc_value(attacker_state.sim_state, n)
...
def _add_paths_to_total_path(
    ...
    for path, _ in sorted(paths, key=itemgetter(1)):
        ...
        added_ttc_cost += sum(ttc_values[step] for step in steps_not_already_in_path[0])
