...
class MalSimGraph(ParallelEnv[str, MALObsInstance, np.int64]):
    ...
    def __init__(self, simulator: MalSimulator, attacker_visible_defense_steps: bool = False):
        ...
        self.lang_serializer = LangSerializer(self.attack_graph.lang_graph, split_assoc_types=False, split_step_types=True)
    ...
    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None) -> tuple[dict[str, MALObsInstance], dict[str, dict[str, Any]]]:
        ...
        self._obs = {
            agent_name: (
                full_obs2attacker_obs(self._full_obs, state, self.lang_serializer, see_defense_steps=self.see_def_steps)
                if isinstance(state, MalSimAttackerState) else
                full_obs2defender_obs(self._full_obs, state, self.lang_serializer)
            )
