...
class LangSerializer:
    ...
    def __init__(
        ...
        self.asset_type: dict[str, int] = {
            asset_type: i
            for i, asset_type in enumerate(sorted(self._lang.assets.keys()))
        }
        ...
        self.step_type2attacker_step_type: NDArray[np.int64] = np.zeros(
            (len(self.step_type),), dtype=np.int64
        )
        for attacker_step_key, idx in self.step_type.items():
            self.step_type2attacker_step_type[idx] = self.attacker_step_type[
                attacker_step_key
            ]

        self.step_type2defender_step_type: NDArray[np.int64] = np.zeros(
            (len(self.step_type),), dtype=np.int64
        )
        for defender_step_key, idx in self.step_type.items():
            self.step_type2defender_step_type[idx] = self.defender_step_type[
                defender_step_key
            ]
