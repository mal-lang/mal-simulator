...
class AssetThenActionWrapper(
    ...
    def __init__(
        self,
        env: Env[MALObsInstance, np.int64],
        model: Model,
    ...
    def step(
        ...
        step_idx = self._obs.step2asset[
            0, np.where(self._obs.step2asset[1] == asset_idx)
        ]
...
class ActionThenAssetWrapper(
    ...
    def step(
        ...
        step_idx = self._obs.step2asset[
            0,
            np.isin(self._obs.step2asset[0], step_idx)
            & np.isin(self._obs.step2asset[1], asset_idx),
        ]
