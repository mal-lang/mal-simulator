...
def node_reward(self, node: AttackGraphNode, agent_name: Optional[str] = None) -> float:
    agent_settings = self._agent_settings[agent_name] if agent_name else None
...
def _defender_observed_nodes(
    ...
    false_negatives = self._generate_false_negatives(defender_name, compromised_nodes)
...
def _attacker_overriding_ttc_settings(
    ...
    ttc_overrides_names = attacker_settings.ttc_overrides.per_node(self.attack_graph)
...
def _get_attacker_agents(self, only_alive: bool = False) -> list[MalSimAttackerState]:
...
def _get_defender_agents(self, only_alive: bool = False) -> list[MalSimDefenderState]:
...
def _defender_step(
    ...
    for node in nodes:
        ...
        if node not in agent.action_surface:
            ...
            self._graph_state.viability_per_node, made_unviable = make_node_unviable(
                node,
...
def _attacker_step_reward(
    ...
    performed_steps = self.performed_attacks_func(attacker_state)
    action = attacker_state.step_attempted_nodes

    # Attacker is rewarded for compromised nodes
    step_reward = sum(self.node_reward(n, attacker_state.name) for n in performed_steps)
    ...
    if self.sim_settings.ttc_mode != TTCMode.DISABLED:
        ...
        for node in performed_steps:
            if reward_mode == RewardMode.EXPECTED_TTC:
                ...
                step_reward -= (
                    TTCDist.from_node(node).sample_value(self.rng) if node.ttc else 0
                )
...
def _defender_step_reward(
    ...
    compromised_nodes = self.enabled_attacks_func(defender_state)
