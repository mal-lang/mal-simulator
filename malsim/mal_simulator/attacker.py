...
from malsim.mal_simulator.agent_state import create_attacker_state
...
def attacker_step_reward(
    ...
    step_reward = sum(
        node_reward(sim_data, n, agent_settings)
...
def attacker_step(
    ...
    for node in nodes:
        ...
        if node in agent.entry_points:
            ...
            can_compromise = node_is_viable(graph_state, node)
        else:
            # Otherwise it is limited by traversability
            can_compromise = node_is_traversable(
                graph_state, agent.performed_nodes, node
            )

        if can_compromise:
            if attempt_attacker_step(agent, graph_state, node, ttc_mode, rng):
...
def initial_attacker_state(
    ...
    ttc_overrides, ttc_value_overrides, impossible_steps = (
        attacker_overriding_ttc_settings(
            attack_graph, attacker_settings, sim_settings, rng
        )
    )
...
def register_attacker_settings(
    ...
    assert attacker_settings.name not in agent_data.agent_settings, (
        f'Duplicate agent named {attacker_settings.name} not allowed'
    )
    agent_data.alive_agents.add(attacker_settings.name)
    agent_data.agent_settings[attacker_settings.name] = attacker_settings

    agent_state = initial_attacker_state(
        graph_state, sim_data, attack_graph, attacker_settings, sim_settings, rng
    )
    agent_data.agent_states[attacker_settings.name] = agent_state
    agent_data.agent_rewards[attacker_settings.name] = attacker_step_reward(
        sim_data,
        agent_state,
    ...
    if len(get_defender_agents(agent_data)) > 0:
...
def attempt_attacker_step(
    ...
    if ttc_mode == TTCMode.DISABLED:
        ...
        node_current_ttc_value = ttc_dist.sample_value(rng)
        return node_current_ttc_value <= 1
    ...
    # or presampled ttcs in PRE_SAMPLE mode
        ...
        node_current_ttc_value = node_ttc_value(graph_state, node, agent)
        return num_attempts + 1 >= node_current_ttc_value

    else:
        raise ValueError(f'Invalid TTC mode: {malsim.sim_settings.ttc_mode}')
