...
class MalSimGUIClient:
    ...
    def _send_request(
        ...
        if method == 'GET':
            ...
            res = requests.post(url, json=json_content, timeout=DEFAULT_TIMEOUT)
        else:
    ...
    def upload_model(self, model: Model) -> None:
        ...
        self._send_request('POST', 'model', json_content=model._to_dict())

    def upload_attack_graph(self, attack_graph: AttackGraph) -> None:
        """Uploads a serialized graph to the POST endpoint of the API"""
        self._send_request('POST', 'attack_graph', json_content=attack_graph._to_dict())
    ...
    def upload_performed_nodes(
        ...
        self._send_request(
            ...
            'performed_nodes',
            json_content=[
                {'node_id': n.id, 'iteration': iteration} for n in new_performed_nodes
            ],
        )

    def upload_latest_attack_steps(
        self, latest_steps: dict[int, list[dict[str, Any]]]
    ) -> None:
        """Upload dict of latest attack step ids mapped to alert logs"""
        self._send_request('POST', 'latest_attack_steps', json_content=latest_steps)

    def upload_defender_suggestions(
        self, suggestions: dict[str, dict[int, dict[Any, Any]]]
    ) -> None:
        """
        Uploads dict of suggestions that maps agent name
        to a dict mapping node id to suggestion info.
        """
        self._send_request('POST', 'defender_suggestions', json_content=suggestions)

    def get_defender_action(self) -> Any:
        """Get selected defender action from API"""
        return self._send_request('GET', 'defender_action').json()

    def clear_defender_action(self) -> None:
        """Post None to selected action from API"""
        self._send_request(
            'POST', 'defender_action', json_content={'iteration': -1, 'node_id': None}
        )

    def get_reward_value(self) -> Any:
        """Get reward value for the current iteration from API"""
        return self._send_request('GET', 'reward_value').json()
    ...
    def upload_initial_state(self, attack_graph: AttackGraph) -> None:
